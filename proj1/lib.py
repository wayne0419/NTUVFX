...
def solve_debevec(photo_list, picked_pixels, channel, L = 20):
	...
	for i in range(N):
		for j in range(P):
			...
			# 	print("Photo " + str(j))
			# 	print("Pixel " + str(i))
			# 	print("Pixel: " + str(pixel.i) + " " + str(pixel.j))
			# 	print("Luminosity: " + str(photo.luminosity[pixel.i, pixel.j, 0]))
			# 	print("dt " + str(photo.dt))
			# 	print(z_ij, w, ln_dt)
			# 	print(A_256.shape)
			# 	print(A_256)
			# 	print(A_N.shape)
...
def merge_irradiance(photo_list, g, channel):
	...
	# 	for j in range(width):
	# 		z = img[i,j]
	# 		irradiance[i,j] = np.exp(g[z] - ln_dt)
	# # ################################

	
	
	# test
	# utilities.show_image(irradiance)
	# print(irradiance)
	# print(irradiance.shape)
	# print(np.max(irradiance, axis=(0,1)))
	# print(np.min(irradiance, axis=(0,1)))
	# print(np.median(irradiance, axis=(0,1)))
	#####################################
